...
public class HttpProxyCacheServer {
   ...
   * <p> If file for this url is fully cached (it means method {@link #isCached(String)} returns
   * {@code true}) then file:// uri to cached file will be returned. <p> Calling this method has
   * same effect as calling {@link #getProxyUrl(String, boolean)} with 2nd parameter set to {@code
   * true}.
   *
  ...
  /**
   * Returns url that wrap original url and should be used for client (MediaPlayer, ExoPlayer, etc).
   * <p> If parameter {@code allowCachedFileUri} is {@code true} and file for this url is fully
   * cached (it means method {@link #isCached(String)} returns {@code true}) then file:// uri to
   ...
   * @return observable that will emmit the cached file path after fetching
   */
  public io.reactivex.Observable<String> fetch(final String url)
      ...
      public void subscribe(@NonNull ObservableEmitter<String> emitter) throws Exception {
        ...
        try {
          ...
          if (!emitter.isDisposed()) {
            emitter.onError(
                new ProxyCacheException(String.format("Failed while reading bytes from %s", url),
                    e));
          }
        } finally {
          if (is != null) {
          ...
          if (!emitter.isDisposed()) {
            emitter.onNext(getProxyUrl(url));
            emitter.onComplete();
          }
        }
      }
  ...
  private String appendToProxyUrl(String url) {
    return String
        .format(Locale.US, "http://%s:%d/%s", PROXY_HOST, port, ProxyCacheUtils.encode(url));
  }
  ...
  public static final class Builder {
    ...
    /**
     * Overrides default cache folder to be used for caching files. <p> By default AndroidVideoCache
     * uses '/Android/data/[app_package_name]/cache/video-cache/' if card is mounted and app has
     * appropriate permission or 'video-cache' subdirectory in default application's cache directory
     * otherwise. </p> <b>Note</b> directory must be used <b>only</b> for AndroidVideoCache files.
     *
